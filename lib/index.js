...
module.exports = function loader(content) {
  ...
  var createWebpFile = function createWebpFile(_ref3) {
    var data = _ref3.data,
        width = _ref3.width,
        height = _ref3.height;
    ...
    var fileNameWebp = fileName.replace(/(jpe?g|png|svg|gif)$/ig, 'webp');

    var _getOutputAndPublicPa3 = getOutputAndPublicPath(fileNameWebp, config),
  ...
  var createPlaceholder = function createPlaceholder(_ref4) {
    var data = _ref4.data;
  ...
  return img.metadata().then(function (metadata) {
    ...
    var adapterWebpOptions = adapterOptions;
    ...
    Promise.all(promises).then(function (results) {
      ...
      return Promise.all(promisesWebp).then(function (results) {
        return firstPass['webpFiles'] = results.slice(0, -1).map(createWebpFile);
      });
    });
  }).then(function (_ref5) {
    var files = _ref5.files,
        webpFiles = _ref5.webpFiles,
        placeholder = _ref5.placeholder;

    var srcset = files.map(function (f) {
      return f.src;
    }).join('+","+');
    var srcsetWebp = webpFiles.map(function (f) {
      return f.src;
    }).join('+","+');
    ...
    loaderCallback(null, 'module.exports = {' + 'srcSetWebP:' + srcsetWebp + ',' + 'srcSet:' + srcset + ',' + 'images:[' + images + '],' + 'src:' + firstImage.path + ',' + 'toString:function(){return ' + firstImage.path + '},' + 'placeholder: ' + placeholder + ',' + 'width:' + firstImage.width + ',' + 'height:' + firstImage.height + '};');
  }).catch(function (err) {
